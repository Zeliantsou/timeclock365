<?php

namespace App\Form;

use App\Entity\Book;
use App\Form\EventListener\Book\AddAuthorsFieldSubscriber;
use App\Form\EventListener\Book\AddDescriptionFieldSubscriber;
use App\Form\EventListener\Book\AddPublishedYearFieldSubscriber;
use App\Form\EventListener\Book\AddTitleFieldSubscriber;
use Symfony\Component\Form\AbstractType;
use Symfony\Component\Form\FormBuilderInterface;
use Symfony\Component\OptionsResolver\OptionsResolver;

class BookType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ->addEventSubscriber(new AddTitleFieldSubscriber())
            ->addEventSubscriber(new AddAuthorsFieldSubscriber())
            ->addEventSubscriber(new AddDescriptionFieldSubscriber())
            ->addEventSubscriber(new AddPublishedYearFieldSubscriber())
        ;
    }

    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            'csrf_protection' => false,
            'data_class' => Book::class,
        ]);
    }
}
